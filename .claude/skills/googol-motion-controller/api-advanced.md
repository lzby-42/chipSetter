...
GTN_RN_ClearStationSafeModeStatus(core, station);             // Manual clear
...
Trigger-based position capture (latch on external signal):
```c
GTN_SetTriggerMode(core, trigger, &mode);
GTN_GetTriggerPos(core, trigger, &pos);
GTN_GetTriggerStatus(core, trigger, &status);
```
Controller and network module resources vary — check `编程手册/高速硬件捕获/` for specific resource tables.
